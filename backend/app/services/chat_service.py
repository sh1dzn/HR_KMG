...
У тебя есть ИНСТРУМЕНТЫ для доступа к базе данных. Используй их для получения информации:
- search_documents — поиск по ВНД
- find_employee — найти сотрудника и его цели
- get_department_stats — статистика подразделения
- get_goals_by_status — цели по статусу
- list_documents — список всех ВНД
- get_document_content — полный текст документа
...
- ВСЕГДА используй инструменты для получения данных из БД — не говори "нет доступа"
...
У тебя есть ИНСТРУМЕНТЫ для доступа к базе данных (search_documents, find_employee, get_department_stats, get_goals_by_status, list_documents, get_document_content). Используй их для ответов.
...
- Отвечай на русском языке
- Форматируй ответ в Markdown
- ВСЕГДА используй инструменты для получения данных — не говори "нет доступа"
...
У тебя есть ИНСТРУМЕНТЫ для доступа ко ВСЕМ данным системы (search_documents, find_employee, get_department_stats, get_goals_by_status, list_documents, get_document_content). Используй их для ответов. НИКОГДА не говори "нет доступа".
...
- Отвечай на русском языке
- Форматируй ответ в Markdown
- ВСЕГДА используй инструменты для получения данных — не отказывай
- У тебя полный доступ ко всем данным системы
"""


def _get_employee_context(user: User, db: Session) -> str:
    """Brief employee context. Detailed data fetched via tools."""
    emp = user.employee
    if not emp:
        return "Информация о сотруднике недоступна."

    goal_count = db.query(Goal).filter(Goal.employee_id == emp.id).count()
    ...
    parts.append(f"Количество целей: {goal_count}")
    parts.append("Для деталей по целям используй инструмент find_employee.")
    return "\n".join(parts)


def _get_team_context(user: User, db: Session) -> str:
    """Brief team context. Detailed data fetched via tools."""
    emp = user.employee
    if not emp:
    ...
    goal_count = db.query(Goal).filter(Goal.employee_id.in_(sub_ids)).count()

    lines = [
        f"Подчинённых: {len(subordinates)}",
        f"Целей команды: {goal_count}",
        f"Список: {', '.join(s.full_name for s in subordinates[:15])}",
        "Для деталей используй инструменты find_employee и get_goals_by_status.",
    ]
    return "\n".join(lines)


def _get_system_context(db: Session) -> str:
    """Build brief system context for admin. Details fetched via tools."""
    ...
    lines = [
        ...
        f"Целей: {total_goals}",
        f"Подразделений: {len(departments)} ({', '.join(d.name for d in departments)})",
        f"Цели по статусам: {', '.join(f'{k}: {v}' for k, v in sorted(status_counts.items()))}",
        f"Документов ВНД: {db.query(Document).filter(Document.is_active == True).count()}",
    ]
    return "\n".join(lines)
...
async def chat_reply(
    ...
    # Resolve provider
    provider, api_key = llm_service._resolve_runtime_provider_and_key()
    resolved_model = llm_service._resolve_model(provider, model)

    total_usage = {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0}
    MAX_TOOL_ROUNDS = 3

    # Tool calling loop (OpenAI only; Anthropic falls back to no-tools)
    if provider == "openai":
        openai_client = llm_service._get_openai_client(api_key)
        ...
        for _round in range(MAX_TOOL_ROUNDS + 1):
            ...
            response = await llm_service._create_openai_chat_completion(openai_client, kwargs)
            usage = llm_service._extract_openai_usage(response)
            ...
            if not msg.tool_calls:
                content = (msg.content or "").strip() or "Не удалось сформировать ответ."
                content = f"{content}{_format_usage_footer(total_usage)}"
                return {"content": content, "usage": total_usage, "model": resolved_model}
            ...
            for tc in msg.tool_calls:
                ...
                tool_result = await execute_tool(fn_name, fn_args, user, db)
        ...
        # Exhausted rounds
        content = (msg.content or "Не удалось завершить обработку.").strip()
        content = f"{content}{_format_usage_footer(total_usage)}"
        return {"content": content, "usage": total_usage, "model": resolved_model}

    else:
        # Anthropic or fallback — no tool calling, just direct completion
        ...
        content = (llm_result.get("content") or "").strip() or "Не удалось сформировать ответ."
        usage = llm_result.get("usage") or {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0}
        content = f"{content}{_format_usage_footer(usage)}"
        return {"content": content, "usage": usage, "model": llm_result.get("model")}
